...
\section{Trabalhos Correlatos}\label{chap:projetos_correlatos}
   Nesta seção, são revisados trabalhos que empregam métodos de verificação formal — tanto dedutivos quanto baseados em interpretação abstrata — utilizando a plataforma Frama-C e seus \emph{plugins}: WP, RTE e EVA.

Em um primeiro momento, são apresentados trabalhos focados na corretude funcional de software, isto é, na prova formal de que uma implementação está em conformidade com sua especificação. Em seguida, são discutidos trabalhos voltados à verificação da ausência de erros em tempo de execução, domínio no qual a interpretação abstrata e ferramentas como o EVA têm se mostrado particularmente eficazes.

\subsection{Corretude Funcional}

A verificação de corretude funcional tem ganhado destaque especialmente em setores de alta criticidade, como o aeronáutico, automotivo e espacial, impulsionada tanto pela maturidade dos métodos
formais quanto pelo surgimento de ferramentas acessíveis como Frama-C. O \emph{plugin} WP, baseado em métodos dedutivos e na teoria da pré-condição mais fraca
\cite{hoare1969axiomatic,dijkstra1970ewd249}, constitui atualmente uma das principais abordagens open-source para prova de programas C de forma modular e orientada por contratos ACSL
\cite{framaCacsl}.

Um dos trabalhos mais influentes que utilizam o WP para provar a corretude funcional de rotinas aeronáuticas é a verificação da biblioteca matemática do piloto automático Paparazzi, conduzida por
Perrone et al. \cite{perrone2016verifying}. Nesse estudo, duas funções de navegação — \emph{float\_rmat\_of\_quat} e \emph{float\_quat\_of\_rmat} — foram formalmente especificadas e
analisadas. Enquanto a primeira pôde ser completamente provada com \emph{solvers} automáticos, a segunda exigiu provas interativas, ilustrando um ponto recorrente na literatura: a necessidade de
conhecimento especializado quando a verificação automática não é suficiente, conforme também discutido por Yaglamunov em sua investigação sobre provas formais de funções da biblioteca padrão
\cite{yaglamunov2021correctness}.

Outro exemplo relevante é o caso de verificação formal apresentado por Silva et al. \cite{silva2016formal}, onde aplicou o método de verificação dedutiva ao software de um subsistema espacial utilizando o
\emph{plugin} Jessie, o antecessor do WP no ecossistema Frama-C. O estudo avaliou funções que realizavam tanto operações inteiras quanto de ponto flutuante. Os autores relatam que, para operações
inteiras, a verificação foi majoritariamente bem-sucedida, com praticamente todas as obrigações de prova sendo resolvidas automaticamente pelos \emph{solvers}. Em contraste, funções envolvendo
aritmética em ponto flutuante — como normalização e manipulação vetorial — apresentaram dificuldades significativas: diversas propriedades não puderam ser provadas de forma automática e no caso do
trabalho em questão a utilização de \emph{proof-assistants} (no qual é possível inserir provas de forma manual e colaborativa) não era uma opção viável considerando o esforço necessário de alteração
da estrutura do código, evidenciando as limitações conhecidas da verificação dedutiva sobre \emph{floating-point}. Além disso,
por depender de funções externas do sistema operacional de tempo real (RTOS), o estudo recorreu à criação especificações em ACSL e implementação da definição da função (tipo de retorno, nome e argumentos) para modelar apenas os comportamentos relevantes dessas chamadas. Essa abstração permitiu a
continuidade da verificação, mas também reforçou o cuidado necessário para garantir que tais modelos não introduzissem inconsistências. Assim, o trabalho demonstra tanto o potencial quanto os desafios
práticos da verificação dedutiva com Jessie/WP, especialmente diante de aritmética de ponto flutuante e dependências externas.

Do ponto de vista conceitual, a consolidação da plataforma Frama-C e o avanço educacional em torno de WP foram impulsionados por obras como o tutorial de Blanchard \cite{blanchard2020introduction} e o
guia de Kosmatov et al. \cite{kosmatov2024guide}, que fornecem bases metodológicas e aplicações práticas para engenheiros e pesquisadores. Esses materiais se tornaram fundamentais para a disseminação
da técnica, especialmente em projetos acadêmicos de verificação orientada a contratos.

\subsection{Ausência de Erros em Tempo de Execução}

A análise de ausência de erros em tempo de execução (RTE) é um dos pilares da conformidade com normas de software crítico, uma vez que falhas como \emph{overflows}, desvios inválidos, violações de
memória e acessos fora dos limites estão entre as principais causas de incidentes históricos, incluindo falhas catastróficas como o caso do Ariane 5 \cite{ariane_5_report}. No contexto do Frama-C,
essa categoria de verificação é tradicionalmente associada ao \emph{plugin} RTE, que gera automaticamente anotações ACSL capazes de identificar potenciais erros, e ao \emph{plugin} EVA, baseado em
interpretação abstrata.

O uso do RTE é ilustrado por Pollien no mesmo estudo sobre a biblioteca Paparazzi \cite{perrone2016verifying}: ali, o autor observa que o RTE é capaz de detectar operações aritméticas potencialmente
inseguras quando os contratos formais não impõem restrições suficientes, porém apresenta limitações para expressões envolvendo estruturas complexas, sobretudo quando efeitos colaterais estão
presentes. Esse comportamento também é relatado por Todorov em sua tese no contexto automotivo \cite{todorov2020automotive}, onde o uso de tipos compostos e arquiteturas orientadas a modelos gerou
cenários que o RTE não conseguiu analisar adequadamente.

Como alternativa mais robusta, o EVA emerge como uma solução baseada em interpretação abstrata capaz de explorar conjuntos de valores possíveis para variáveis e estados de execução
\cite{framaC_eva_manual}. Diferentemente do WP, que depende de especificações completas, o EVA opera de forma mais automatizada e é adequado para análise de grandes bases de código, tornando-se
comparável a ferramentas industriais como Astrée \cite{kaestner2010astree}, tradicionalmente utilizada para garantir ausência de erros em software aeronáutico certificado.


Estudos recentes, como a análise conduzida por Virgile em \cite{Virgile2025_framaC_EVA}, demonstram tanto a eficácia quanto os limites do EVA na propagação de intervalos e no tratamento de contratos
ACSL. Esses casos práticos reforçam a necessidade de uma escrita precisa de especificações para que a análise abstrata possa reduzir imprecisões (\emph{false alarms}) sem comprometer a completude da
verificação.


Por fim, vale ressaltar que o cruzamento entre WP, RTE e EVA é parte de uma tendência já observada em estudos mais amplos sobre verificação formal, que defendem a complementaridade entre métodos de
prova e análise automática \cite{cofer2012,rushby2011,delmas2012}. A integração dessas abordagens, especialmente em fluxos de desenvolvimento exigidos por DO-178C/DO-333, forma hoje a base das
práticas modernas de verificação em software crítico.
