...
\vspace{2cm}
\begin{center}
    \textbf{Verificação  por Métodos Formais da Biblioteca CMSIS-DSP ARM}
\end{center}
...
\section{Introdução}
    ...
    Considerando esse contexto, propõe-se neste trabalho a verificação por métodos formais da biblioteca otimizada de computação para sistemas ARM embarcados CMSIS-DSP, através de ferramentas open-source (Frama-C e seus diferentes \emph{plugins}). Além disso, o framework desenvolvido será empregado na validação de uma implementação de Filtro de Kalman, garantindo então a ausência de \emph{Run Time Errors} e a corretude funcional de tal algoritmo.


    \subsection{Objetivos}
    Propõe-se neste trabalho, a verificação formal da biblioteca CMSIS-DSP, garantindo sua  corretude funcional e a ausência de \emph{runtime errors}. Para tal, será feita:
    \begin{itemize}
        \item A analise do código fonte da biblioteca CMSIS-DSP, mais especificamente do \emph{kernel} de operações matriciais, e a identificação de possíveis fontes de \emph{run-time errors} através do \emph{plugin} RTE.
...
\section{Conceitos e Fundamentação Teórica}\label{chap:revisao_teorica}
    Nesta seção apresentaremos alguns conceitos de fundamental importância para o campo de engenharia de software e em seguida introduziremos teorias fundamentais para a área de verificação formal. 
...
\section{Trabalhos Correlatos}\label{chap:projetos_correlatos}
    Nesta seção, revisaremos brevemente trabalhos que implementaram métodos de verificação formal por métodos dedutivos e por métodos de interpretação abstrata em alguma capacidade utilizando a plataforma Frama-c e seus \emph{plugins}. 
...
\subsubsection{Análise de Possíveis Erros em Tempo de Execução}
\subsubsection{Escrita de Contratos de Funções}
\subsubsection{Escrita de Invariantes}
\subsubsection{Análise dos Casos de Verificação}
